...
## - Save SKU into arrays to check later, take the SKU file with category informations and save each category
...
Category['All'].remove(directory_path + 'SKU_NH-added.csv')
...
for categories in Category:
    ...
    for files in Category[categories]:
        ...
        output = files.replace(" - HMP", '').replace(" - TPCN", '').replace(" - DD", '').replace('csv converted\\',
                                                                                                 'csv converted\\output\\')
        # Open input and output file
        try:
            with open(files, 'r', encoding='ISO-8859-1') as input_files, open(output, 'a', newline='',
